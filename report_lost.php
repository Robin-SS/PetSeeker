...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Validation
    if (empty($name) || $category_id <= 0 || empty($color) || $location_id <= 0 || empty($date_lost)) {
        ...
        if (empty($error)) {
            try {
                ...
                // 1. Insert Pet Record (photo stores the Supabase public URL)
                ...
                $lost_stmt = $pdo->prepare("
                    ...
                    VALUES (?, ?, ?, ?, ?, 'Approved')
                ");
                $lost_stmt->execute([$pet_id, $user_id, $location_id, $date_lost, $additional_notes]);
